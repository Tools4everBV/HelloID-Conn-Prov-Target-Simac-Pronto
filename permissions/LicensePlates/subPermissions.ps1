...
$licensePlate = $personContext.Person.PrimaryContract.Custom.prontoLicensePlate # Mandatory
...
try {
    ...
    # Get IdentificationId for LicensePlate from Simac-Pronto to use as reference for permission granting
    $retrieveLicensePlateSplatParams = @{
        Uri     = "$($actionContext.Configuration.BaseUrl)/api/v1/identifications?filter=$($licensePlate)"
        Method  = 'GET'
        Headers = $headers
    }

    $retrievedLicensePlate = (Invoke-RestMethod @retrieveLicensePlateSplatParams).data | Select-Object -First 1
    if ($null -eq $retrievedLicensePlate) {
        throw "The Simac-Pronto identification with value '$($identificationId)' could not be found. Please make sure the identification exists in Simac-Pronto before granting permissions to it."
    }
    
    $identificationId = $retrievedLicensePlate.id
    ...
    if (-not($actionContext.Operation -eq 'revoke')) {
        ...
        $desiredPermissions["$($licensePlate)"] = "$($identificationId)"
    }
    ...
    foreach ($permission in $desiredPermissions.GetEnumerator()) {
        ...
        if (-not $currentPermissions.ContainsKey($permission.Name)) {
            ...
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    ...
                    Message = "Granted access to permission $($permission.Name) [$($permission.Value)]"
                    IsError = $false
                })
        }
    }
    ...
    foreach ($permission in $currentPermissions.GetEnumerator()) {
        if (-not $desiredPermissions.ContainsKey($permission.Name)) {
            ...
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    ...
                    Message = "Revoked access to permission $($permission.Name) [$($permission.Value)]"
                    IsError = $false
                })
        }
        else {
    ...
    if ($actionContext.Operation -eq 'update') {
        foreach ($permission in $newCurrentPermissions.GetEnumerator()) {
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    ...
                    Message = "No changes to access of permission $($permission.Name) required."
                    IsError = $false
                })
        }
    }
