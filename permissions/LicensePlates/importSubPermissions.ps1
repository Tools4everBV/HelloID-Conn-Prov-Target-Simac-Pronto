####################################################################
# HelloID-Conn-Prov-Target-Simac-Pronto-ImportPermissions-LicensePlates
# PowerShell V2
####################################################################
...
$permissionReference = 'LicensePlate'
$permissionDisplayName = 'LicensePlate'
...
try {
    ...
    do {
        ...
        if ($response.data) {
            $importedAccounts += $response.data | Select-Object -Property Id, Identifications
        }
    ...
    do {
        $splatImportPermissionParams = @{
            Uri     = "$($actionContext.Configuration.BaseUrl)/api/v1/identifications?page=$($pageNumber)&filter=LicensePlate"
            Method  = 'GET'
            Headers = $headers
        }
            ...
            foreach ($importedPermission in $response.data) {
                ...
                if ($null -ne $importedPermission.id) {
                    ...
                    foreach ($account in $importedAccounts) {
                        if ($account.Identifications.id -contains $importedPermission.id) {
                    ...
                    $permission = @{
                        ...
                        SubPermissionDisplayName = "$($importedPermission.SerialNumber)"
                    }
