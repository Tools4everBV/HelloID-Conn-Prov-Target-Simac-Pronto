#################################################################
# HelloID-Conn-Prov-Target-Simac-Pronto-RevokePermission-PersonsGroup
# PowerShell V2
#################################################################
...
try {
    ...
    switch ($action) {
        'RevokePermission' {
            # Set the UntilTime to current date where the PersonsGroup Id matches the permission reference. To revoke the permission.
            $correlatedAccount.PersonsGroups | Where-Object { $_.Id -eq $actionContext.References.Permission.Reference } | ForEach-Object { $_.UntilTime = (Get-Date).ToString('yyyy-MM-ddT00:00:00') }
